...
const DriverTrips = () => {
  ...
  const { data: trips = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return results
        .filter(
          (r): r is PromiseFulfilledResult<MoveTripDetailed> =>
            r.status === "fulfilled",
        )
        .map((r) => r.value);
    },
  ...
  if (isLoading)
    return <LoadingDelight label="Loading your trip assignments..." />;
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1 font-medium">
            Track your upcoming moves and earning history
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-primary/5 border border-primary/10">
          ...
          <span className="text-[10px] font-bold text-primary uppercase tracking-wider">
            ...
            {trips
              ...
              .reduce((s, t) => s + (t.moveOfferPopulated?.price ?? 0), 0)
              .toLocaleString()}
          </span>
        </div>
      </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest">
                      Job: SM-{selectedTrip?.id?.toString().padStart(5, "0")}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                ...
                <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="flex items-center gap-5">
                    <div className="w-14 h-14 rounded-2xl bg-secondary/50 flex items-center justify-center text-lg font-black text-muted-foreground border border-border/50">
                      {selectedTrip?.moveRequestPopulated?.client
                        ?.firstName?.[0] ?? "?"}
                      {selectedTrip?.moveRequestPopulated?.client
                        ?.lastName?.[0] ?? "?"}
                    </div>
                    <div>
                      ...
                      <p className="text-base font-bold text-foreground">
                        {selectedTrip?.moveRequestPopulated?.client
                          ?.firstName ?? "Unknown"}{" "}
                        {selectedTrip?.moveRequestPopulated?.client?.lastName ??
                          "Client"}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground font-medium mt-1">
                        {selectedTrip?.moveRequestPopulated?.hasFurniture
                          ? "Contains Furniture"
                          : "No Large Furniture"}
                      </p>
                    </div>
                  ...
                  <div className="flex gap-10">
                    <div className="space-y-1">
                      ...
                      <p className="text-3xl font-black text-primary tracking-tighter">
                        ${selectedTrip?.moveOfferPopulated?.price ?? 0}
                      </p>
                    </div>
                    <div className="space-y-1">
                      <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
                      ...
                      <p className="text-lg font-bold text-foreground tracking-tight">
                        {selectedTrip?.moveRequestPopulated?.moveDate
                          ? new Date(
                              selectedTrip.moveRequestPopulated.moveDate,
