...
const ClientTrips = () => {
  ...
  const cancelMutation = useMutation({
    mutationFn: (id: number) =>
      tripService.updateTripStatus(String(id), "CANCELLED"),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["clientTrips"] });
      toast({
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1 font-medium">
            Manage your active and past moving experiences
          </p>
        </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest">
                      ID: SM-{selectedTrip?.id?.toString().padStart(5, "0")}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (confirm("Cancel this trip?"))
                          cancelMutation.mutate(selectedTrip.id);
                      }}
                    >
                      Cancel Trip
                    </Button>
                ...
                <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="flex items-center gap-5">
                    <div className="w-14 h-14 rounded-2xl bg-secondary/50 flex items-center justify-center text-lg font-black text-muted-foreground border border-border/50">
                      {selectedTrip?.moveOfferPopulated?.driver?.user
                        ?.firstName?.[0] ?? "?"}
                      {selectedTrip?.moveOfferPopulated?.driver?.user
                        ?.lastName?.[0] ?? "?"}
                    </div>
                  ...
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      ...
                      <p className="text-base font-bold text-foreground">
                        {selectedTrip?.moveOfferPopulated?.vehicle
                          ? getVehicleString(
                              selectedTrip.moveOfferPopulated.vehicle,
                            )
                          : "No vehicle info"}
